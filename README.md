...
| [**Spiral Resume**](apps/Resume/) | Your resume in twelve typeset layouts, as a PDF or a Word file. It tightens the wording and is never allowed to change a fact. | **v0.1.0** — first release, macOS + Windows | [resume-v0.1.0](https://github.com/cococool13/spiral/releases/tag/resume-v0.1.0) |
...
| [`apps/Resume/`](apps/Resume/) | Spiral Resume: a resume goes in, a typeset PDF or Word file comes out, and no fact is ever changed. macOS + Windows. First release **v0.1.0**. Import, the Check screen where every extracted fact is editable, twelve templates rendered by an embedded Typst, PDF and DOCX export, and three engine tiers. 218 Rust tests plus 76 Vitest tests. See the [design spec](apps/Resume/docs/design-spec.md) | shipping the resume app |
...
**Resume** first shipped as **v0.1.0** (`resume-v0.1.0`): import, the Check
...
Installers land on that GitHub release as CI finishes. Homebrew needs a cask
bump in `cococool13/homebrew-spiral` after the DMG exists — that step is not
CI. Do not use `/releases/latest` for Resume; that URL is Wallpaper.
