/**
 * The Spiral mark, assembled from its parts.
 *
 * Path data mirrors /brand/logo/mark-red.svg and the website's HeroLogo, and
 * the entrance is the same one: three machined segments arrive separately —
 * the core first, then the upper and lower bands sliding in to lock against
 * it — followed by one specular sweep across the finished mark.
 *
 * The website drives this with a framer-motion spring. Here it is CSS, and
 * the reason is not weight: framer writes `initial` as an inline style, so
 * anything that stops it running (a missing `LazyMotion`, a failed chunk)
 * leaves `opacity: 0` on the element permanently. That is what happened on
 * the first attempt — an invisible mark on the one screen that has to work.
 * A CSS keyframe's base style is the *finished* state; it departs from it and
 * returns. Motion fails invisible, CSS fails visible.
 *
 * The overshoot curve approximates the website's spring (stiffness 250,
 * damping 19, mass 0.9 — damping ratio ~0.63, so one visible bounce).
 * Timings and travel distances are copied exactly, so the two read as the
 * same move. The website's per-part fade is dropped for the same reason: an
 * `opacity: 0` keyframe is a way for this screen to end up blank. The parts
 * arrive opaque instead. All of it lives in app.css under `intro-mark`.
 */
const VIEWBOX = "337 154 352 566";

const PARTS = [
  {
    part: "core",
    d: "M529.259 337.978C531.149 337.917 539.823 342.101 542.227 343.176L565.834 353.636C575.81 358.069 585.831 362.398 595.896 366.624C607.103 371.227 616.429 374.136 626.727 380.807C643.878 391.918 657.468 408.737 650.84 430.226C642.57 457.039 615.21 472.463 591.975 484.903C572.681 458.156 538.448 450.902 510.297 435.992C510.673 449.177 510.44 463.253 510.554 476.526C510.864 498.047 510.993 519.571 510.939 541.095C508.763 540.243 506.305 539.118 504.125 538.182C474.206 525.739 444.975 511.316 414.678 499.592C399.233 493.615 383.552 484.757 376.377 469.154C371.522 458.458 371.139 446.265 375.312 435.285C381.114 419.787 393.357 413.164 407.017 405.462C415.476 400.692 422.463 397.173 431.284 393.232C448.417 407.993 458.979 412.347 479.189 421.484L509.836 435.63C509.045 426.941 509.377 413.395 509.328 404.435L509.065 348.632C515.793 345.335 522.634 341.539 529.259 337.978Z",
  },
  {
    part: "upper",
    d: "M556.692 174.487L557.044 174.592C557.51 175.745 557.932 298.469 557.739 306.768C518.556 328.362 479.207 349.653 439.694 370.638C420.47 380.993 399.084 391.105 381.252 403.764C371.714 410.534 363.903 420.45 359.371 431.308L359.257 431.547L358.909 431.314C358.121 417.082 358.542 398.38 358.472 383.805L358.33 350.887C358.28 337.345 357.475 325.345 361.09 312.138C364.9 297.735 372.717 284.708 383.633 274.569C393.724 265.211 407.036 258.21 419.074 251.452L449.278 234.549L556.692 174.487Z",
  },
  {
    part: "lower",
    d: "M663.932 440.8L665.658 441.395C665.388 463.86 665.553 486.811 665.581 509.123C665.626 545.117 668.613 578.143 637.226 603.154C618.862 617.788 597.54 627.201 576.807 638.402C554.404 650.358 532.131 662.556 509.991 674.992C495.152 683.2 481.231 690.608 466.512 699.311L466.382 570.818C491.036 557.713 515.265 543.791 539.805 530.554C558.871 520.474 578.016 510.469 596.685 499.664C621.384 485.369 652.41 468.546 663.932 440.8Z",
  },
] as const;

export function IntroMark({ size }: { readonly size: number }) {
  const width = Math.round((size * 352) / 566);

  return (
    <svg
      className="intro-mark"
      width={width}
      height={size}
      viewBox={VIEWBOX}
      fill="none"
      role="img"
      aria-label="Spiral"
      style={{ overflow: "visible" }}
    >
      <defs>
        <linearGradient id="intro-mark-sheen" x1="0" y1="0" x2="1" y2="0">
          <stop offset="0" stopColor="white" stopOpacity="0" />
          <stop offset=".5" stopColor="white" stopOpacity=".5" />
          <stop offset="1" stopColor="white" stopOpacity="0" />
        </linearGradient>
        <mask id="intro-mark-mask" maskUnits="userSpaceOnUse">
          {PARTS.map(({ part, d }) => (
            <path key={part} d={d} fill="white" />
          ))}
        </mask>
      </defs>

      {PARTS.map(({ part, d }) => (
        <path key={part} className="intro-mark__part" data-part={part} d={d} />
      ))}

      {/* One specular sweep across the assembled mark. */}
      <g mask="url(#intro-mark-mask)">
        <rect
          className="intro-mark__sheen"
          x={197}
          y={124}
          width={150}
          height={640}
          fill="url(#intro-mark-sheen)"
        />
      </g>
    </svg>
  );
}
