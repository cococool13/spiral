...
7. **Live thumbnails of the user's document.** The style picker renders the user's own resume in every template (`render_all_thumbnails` / `renderThumbnails(accent, doc)`), at page scale, so choosing a style is choosing how this document looks. *(Supersedes the 0.1.0 fixed-sample ship; the original grilling chose live thumbnails.)*
...
14. **Two result actions.** "Save the PDF or Word file to your computer" and "Try another style", which returns to the picker with data and edits intact. M6 planned a conditional "Rewrite the wording again" with a version strip; that button was never added.
...
17. **One optional local model,** ~2.5 GB, 4B-class instruct, 4-bit quantised, run through a bundled llama.cpp sidecar. The download size is stated before it starts. **Widened 2026-08-13 to three:** the same family at 2B, 4B and 9B, because whether a model is good enough depends on the machine it runs on, and a 1.3 GB download that works beats a 5.7 GB one that swaps. Settings lists them with their sizes and one line each; the user picks, and downloading one chooses it. `docs/offline-model.md` records why they are all Qwen3.5 and why they come from a quantiser rather than the publisher.
...
| 6 | **Result** | The finished document. Save to your computer · Try another style. |
