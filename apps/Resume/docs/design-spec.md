...
7. **Sample thumbnails.** The style picker renders a fixed sample resume in every template, at page scale, so choosing a layout does not wait on typesetting the user's document. The user's facts land on the page at Build. *(Shipped this way in 0.1.0; the original grilling chose live thumbnails of the user's own content.)*
