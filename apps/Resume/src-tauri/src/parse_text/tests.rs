...
/// Dates on their own line, then the office, then the bullets — the most
/// common layout after a Word export. The office used to open a phantom role
/// and take the achievements with it.
#[test]
fn a_location_under_a_bare_date_stays_in_the_same_role() {
    let doc = parse_text(
        "Ada\n\nEXPERIENCE\nAnalyst, Admiralty\nJan 2021 - Present\nLondon, UK\n- Wrote the first algorithm\n",
    );
    assert_eq!(doc.experience.len(), 1, "split into phantom roles");
...
/// Two jobs with dates on the line below each title must still split.
#[test]
fn two_roles_with_dates_on_the_next_line_are_two_roles() {
    let doc = parse_text(
        "Ada\n\nEXPERIENCE\nAnalyst, Admiralty\nJan 2021 - Present\nIntern, Works\nJun 2019 - Dec 2020\n",
    );
    assert_eq!(doc.experience.len(), 2);
...
#[test]
fn an_unmarked_achievement_under_a_bare_date_stays_a_bullet() {
    let doc = parse_text(
        "Ada\n\nEXPERIENCE\nAnalyst, Admiralty\nJan 2021 - Present\nWrote the first algorithm\nCut report turnaround from 9 days to 2\n",
    );
    assert_eq!(doc.experience.len(), 1, "split into phantom roles: {:?}", doc.experience.iter().map(|r| r.title.as_str()).collect::<Vec<_>>());
    assert_eq!(doc.experience[0].title, "Analyst");
    assert!(
        doc.experience[0]
            .bullets
            .iter()
            .any(|b| b.text == "Wrote the first algorithm"),
        "{:?}",
...
/// A degree and a range sharing a line used to keep the dates and drop
/// the qualification.
#[test]
fn a_credential_that_shares_a_line_with_its_dates_is_kept() {
    let doc = parse_text(
        "Ada\n\nEDUCATION\nUniversity of London\nBSc Mathematics, 2016 - 2019\n",
    );
    assert_eq!(doc.education[0].institution, "University of London");
    assert_eq!(doc.education[0].credential, "BSc Mathematics");
    assert_eq!(doc.education[0].start.raw, "2016");
    assert_eq!(doc.education[0].end.raw, "2019");
}
...
/// A short unlabelled profile line is still a sentence, not a city.
#[test]
fn a_short_header_sentence_becomes_the_summary_not_the_location() {
    let doc = parse_text(
        "Ada Lovelace\nSenior analyst open to remote work\nada@example.com\n",
    );
    assert_eq!(doc.contact.location, "");
    assert_eq!(doc.summary, "Senior analyst open to remote work");
}
...
/// A compound heading letter-spaced with the same gap between letters as
/// between words concatenates to one token. It is still the heading.
#[test]
fn a_letter_spaced_compound_heading_still_names_its_section() {
    let doc = parse_text(
    ...
    assert_eq!(
        doc.experience.len(),
        1,
        "WORKEXPERIENCE was not read as a heading"
    );
    assert_eq!(doc.experience[0].organization, "Admiralty");
}
...
#[test]
fn an_all_caps_compound_heading_is_not_split_at_and() {
    let skills = parse_text("Ada\n\nSKILLS AND INTERESTS\nRust, Analysis\n");
    assert_eq!(skills.skills[0].items, vec!["Rust", "Analysis"]);
    let honors = parse_text("Ada\n\nHONORS AND AWARDS\nDean's List\n");
    assert_eq!(honors.awards, vec!["Dean's List"]);
    let lead = parse_text(
        "Ada\n\nLEADERSHIP AND ACTIVITIES\nPresident, Chess Club\n2021 - 2022\n- Ran the league\n",
    );
    assert_eq!(lead.leadership.len(), 1);
    assert_eq!(lead.leadership[0].title, "President");
}
...
#[test]
fn a_hyphenated_numeric_date_range_is_lifted_off_the_heading() {
    let doc = parse_text(
        "Ada\n\nEXPERIENCE\nAnalyst, Admiralty 01-2021 - 03-2023\n- Wrote it\n",
    );
    assert_eq!(doc.experience[0].title, "Analyst");
