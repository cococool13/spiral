...
**Implementation note, 2026-08-04.** The paragraph above states the policy; it is not a description of the shipped catalog. M2 shipped eight entries covering caches, logs and developer artifacts. M3 adds five more: browser caches (Chrome, Brave, Edge, Firefox) and the Trash itself. Safari is intentionally absent; its cache lives in a container protected by macOS beyond Full Disk Access, so the app does not claim to clean it. Developer artifacts are not all "library-resident": `~/.gradle/caches` and `~/.npm/_cacache` sit at the top of the home folder, not under `~/Library`. What qualifies them is that they rebuild offline and hold nothing a person authored, which is the property this policy actually turns on.
