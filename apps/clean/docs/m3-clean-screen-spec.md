...
- **Directory removal on the Clean screen.** `scan` yields `is_file()` entries only, so `commands` builds file candidates only, so a Clean run removes no directory. Emptying the Trash leaves the folder skeleton in Finder, and `~/Library/Caches` keeps the now-empty directories its files sat in — real machines accumulate this residue in the hundreds. Pruning them needs a rule for what may be pruned (a catalog directory this run actually emptied, never one that merely *looks* empty because its contents were excluded or failed), and guards proven by mutation like every other. Recorded here so the limitation is a decision on the record, not an oversight.

  **Amendment, 2026-08-04 (M4): two claims in the paragraph above have been overtaken and are corrected here rather than left to mislead.**

  This section originally said directory pruning "gets its own design and its own review gate rather than arriving as a side effect", and that `Outcome::PartiallyRemoved` cannot arise. Both are now false of the application, though the first sentence of this bullet remains true of the Clean screen.

  M4 introduced **directory removal for app-owned containers** (ADR-0015). `associate::associate` returns `read_dir` entries, so `~/Library/Containers/<id>`, `~/Library/Group Containers/group.<id>` and `<id>.savedState` are directories, and `uninstall_execute` also emits the `.app` bundle itself; `remove::delete_permanent` removes each recursively. **Clean itself still removes only files** — the rule above is unchanged for this screen, and pruning emptied catalog directories is still unbuilt and still needs its own design. What changed is that "no directory is ever removed" was a statement about the whole app, and is no longer one.

  It follows that `Outcome::PartiallyRemoved` **is now reachable** — from an uninstall, never from a Clean run. It was always reported in its own bucket, worded as a partial removal and never under "could not be removed", precisely so that the first producer of directory candidates would not have to discover the report was lying about it. That producer has landed and the report was not lying.
