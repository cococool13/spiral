...
- **Brand:** consumed from `brand/` at build time via `node scripts/sync-brand.mjs clean` into gitignored `src/styles/tokens.css` and `src/assets/brand/`. `check-hex.mjs` gates the build. No brand value is ever defined inside `apps/clean/`.
...
3. **Catalog families:** app/system caches and logs · browser caches · Trash · developer artifacts.
   ...
   **Updated 2026-08 (post-M3).** The catalog in `src-tauri/src/catalog.rs` now covers caches and logs (`user-caches`, `icon-services-cache`, `user-logs`, `crash-reports`, `saved-state`), developer artifacts (`xcode-derived-data`, `ios-device-support`, `simulator-caches`, `package-manager-caches`), named browser caches (`chrome-cache`, `brave-cache`, `edge-cache`, `firefox-cache`), and Trash (`trash`). Browser entries name specific per-browser cache paths only — never a full profile directory. Trash is the recoverable destination for other removals (ADR-0001); emptying it is a separate, labeled act.
